...
expName = 'Bernhard'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\sarah\\Desktop\\Uni Graz\\Arbeit\\Doktorat\\MRT Seminar\\MCT neu Experiment\\MCTins_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    ins_img = visual.ImageStim(
        win=win,
        name='ins_img', units='height', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.33, 1),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    ins_key = keyboard.Keyboard(deviceName='defaultKeyboard')
    ins_hint = visual.TextStim(win=win, name='ins_hint',
        text='',
        font='Arial',
    ...
    ins_loop = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('stim/MCT_ins.csv'), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisIns_loop in ins_loop:
        ...
        ins = data.Routine(
            name='ins',
            components=[ins_img, ins_key, ins_hint],
        )
        ins.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ins_img.setImage(slide)
        # create starting attributes for ins_key
        ins_key.keys = []
        ins_key.rt = []
        _ins_key_allKeys = []
        ins_hint.setText(ins_hint_txt)
        ...
        while continueRoutine:
            ...
            # if ins_img is starting this frame...
            if ins_img.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ins_img.frameNStart = frameN  # exact frame index
                ins_img.tStart = t  # local t and not account for scr refresh
                ins_img.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(ins_img, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ins_img.started')
                # update status
                ins_img.status = STARTED
                ins_img.setAutoDraw(True)
            
            # if ins_img is active this frame...
            if ins_img.status == STARTED:
                # update params
                pass
            
            # if ins_img is stopping this frame...
            if ins_img.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > ins_img.tStartRefresh + ins_dur-frameTolerance:
                    # keep track of stop time/frame for later
                    ins_img.tStop = t  # not accounting for scr refresh
                    ins_img.tStopRefresh = tThisFlipGlobal  # on global time
                    ins_img.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'ins_img.stopped')
                    # update status
                    ins_img.status = FINISHED
                    ins_img.setAutoDraw(False)
            ...
            # if ins_key is stopping this frame...
            if ins_key.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > ins_key.tStartRefresh + ins_dur-frameTolerance:
                    # keep track of stop time/frame for later
                    ins_key.tStop = t  # not accounting for scr refresh
                    ins_key.tStopRefresh = tThisFlipGlobal  # on global time
                    ins_key.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'ins_key.stopped')
                    # update status
                    ins_key.status = FINISHED
                    ins_key.status = FINISHED
            if ins_key.status == STARTED and not waitOnFlip:
                theseKeys = ins_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            # if ins_hint is starting this frame...
            if ins_hint.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ins_hint.frameNStart = frameN  # exact frame index
                ins_hint.tStart = t  # local t and not account for scr refresh
                ins_hint.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(ins_hint, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'ins_hint.started')
                # update status
                ins_hint.status = STARTED
                ins_hint.setAutoDraw(True)
            
            # if ins_hint is active this frame...
            if ins_hint.status == STARTED:
                # update params
                pass
            
            # if ins_hint is stopping this frame...
            if ins_hint.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > ins_hint.tStartRefresh + ins_dur-frameTolerance:
                    # keep track of stop time/frame for later
                    ins_hint.tStop = t  # not accounting for scr refresh
                    ins_hint.tStopRefresh = tThisFlipGlobal  # on global time
                    ins_hint.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'ins_hint.stopped')
                    # update status
                    ins_hint.status = FINISHED
                    ins_hint.setAutoDraw(False)
        ...
        if ins_key.keys in ['', [], None]:  # No response was made
            ins_key.keys = None
        ins_loop.addData('ins_key.keys',ins_key.keys)
        if ins_key.keys != None:  # we had a response
            ins_loop.addData('ins_key.rt', ins_key.rt)
            ins_loop.addData('ins_key.duration', ins_key.duration)
    ...
    continueRoutine = True
    # update component parameters for each repeat
    t_img.setImage('stim/Items/Party_Pic.jpg')
    t_audio.setSound('stim/Items/anonymity_1_high.mp3', secs=25.3, hamming=True)
    ...
    while continueRoutine:
        ...
        if t_audio.status == NOT_STARTED and tThisFlip >= 30-frameTolerance:
            # keep track of start time/frame for later
        ...
        if t_audio.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > t_audio.tStartRefresh + 25.3-frameTolerance or t_audio.isFinished:
                # keep track of stop time/frame for later
